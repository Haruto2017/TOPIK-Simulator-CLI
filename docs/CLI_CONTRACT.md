...
- `/compose [topic] [count]` (alias `/write`): sentence-writing practice — an English sentence is shown and you type the Korean. An exact match (whitespace/trailing-punctuation tolerant, against any accepted answer) auto-passes; otherwise the model answer and variants are revealed and you self-rate `y`/`n`. `topic` focuses one topic (e.g. greetings, travel); `count` defaults to 10. `/say` reads the model aloud; `/pause` stops with a summary. Sentences come from the `content/sentences/` directory (one file per topic).
