...
## Cold Storage: Opus Compression

When `ffmpeg` is on PATH, `topik-sim audio compress [--older-than-days <n>] [--bitrate 24k]` transcodes cold WAVs to Opus (~10x smaller) and deletes the WAV. The cache then holds a mix of hot `.wav` and cold `.opus` entries:

- Playback always uses WAV (`Media.SoundPlayer` requirement). When a needed entry exists only as `.opus`, `synthesize_many` restores it through ffmpeg back into the cache and deletes the opus — compressed entries promote to hot automatically on use.
- Without ffmpeg, a compressed entry is simply regenerated by the TTS provider; nothing breaks.
- `audio stats` reports the wav/opus split; `audio prune` evicts both kinds by LRU.
