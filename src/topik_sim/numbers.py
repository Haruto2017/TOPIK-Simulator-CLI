...
# Which system a context wants — the choice students find hardest.
USAGE_GUIDE = [
    ("Dates, years, months", "Sino", "유월 십오일 — June 15th (유월/시월 are irregular)"),
    ("Money (원)", "Sino", "오천삼백 원 — ₩5,300"),
    ("Minutes, phone, floors, math", "Sino", "십오 분 · 공일공 · 삼 층 · 이 더하기 삼"),
    ("Counting things (+ counter)", "Native", "사과 두 개 — two apples"),
    ("Age (살)", "Native", "스무 살 — twenty years old"),
    ("The hour (시)", "Native", "세 시 십오 분 — 3:15 (native hour, Sino minute)"),
]


def cheat_sheet() -> dict:
    """Both systems as reference tables, generated by the same converters the
    drill grades with — so the sheet can never disagree with the grader."""
    sino_values = list(range(1, 11)) + [20, 30, 100, 1000, 10000]
    native_values = list(range(1, 11)) + [20]
    native_rows = []
    for value in native_values:
        row = {"n": value, "reading": native_korean(value)}
        counter = native_counter(value)
        if counter != row["reading"]:
            row["counter_form"] = counter
        native_rows.append(row)
    return {
        "sino": [{"n": value, "reading": sino_korean(value)} for value in sino_values],
        "native": native_rows,
        "counters": [{"counter": counter, "meaning": gloss} for counter, gloss in _COUNTERS],
        "usage": [{"context": c, "system": s, "example": e} for c, s, e in USAGE_GUIDE],
    }
